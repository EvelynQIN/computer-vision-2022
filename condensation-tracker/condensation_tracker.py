...
def condensation_tracker(video_name, params):
    ...
    particles = np.tile(mean_state_a_priori[0], (params["num_particles"], 1)) # (n_particles, len_state)
    particles_w = np.ones([params["num_particles"], 1]) * 1./params["num_particles"] # (n_particles, 1)
    ...
    for i in range(last_frame - first_frame + 1):
        ...
        # if params["draw_plots"] and t == 36:
        #     plt.savefig('./results/appearance/v1_{}p_{}b_{}sp_{}sv_{}so_{}a_frame{}.png'.format(params['num_particles'], params['hist_bin'], params["sigma_position"], params["sigma_velocity"],params["sigma_observe"], params["alpha"], t))
...
if __name__ == "__main__":
    ...
    params = {
        ...
        "hist_bin": 8, # 16
        "alpha": 0, # 0
        "sigma_observe": 0.2, #0.1
        "model": 1,
        "num_particles": 60, # 30
        "sigma_position": 10, #15
        "sigma_velocity": 0.5, #1
