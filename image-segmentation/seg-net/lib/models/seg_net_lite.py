...
class SegNetLite(nn.Module):
    ...
    def __init__(self, kernel_sizes=[3, 3, 3, 3], down_filter_sizes=[32, 64, 128, 256],
        ...
        layers_conv_down = [nn.Conv2d(input_size, down_filter_sizes[0], kernel_sizes[0], 1, conv_paddings[0])] # down_conv1
        for i in range(self.num_down_layers - 1):
            layers_conv_down.append(
                nn.Conv2d(
                    down_filter_sizes[i], 
                    down_filter_sizes[i+1], 
                    kernel_sizes[i+1], 
                    1, 
                    conv_paddings[i+1]))
        ...
        layers_conv_up = [nn.Conv2d(down_filter_sizes[-1], up_filter_sizes[0], kernel_sizes[0], 1, conv_paddings[0])] # up_conv1
    ...
    def forward(self, x):
        ...
        for i in range(self.num_down_layers):
            ...
            x = self.relu(x)
            x, ind = self.layers_pooling[i](x) # also return the indices of the max values
            maxpool_indices.append(ind)
        
        # upsampling
        for i in range(self.num_up_layers):
            x = self.layers_unpooling[i](x, maxpool_indices[-(1+i)])
