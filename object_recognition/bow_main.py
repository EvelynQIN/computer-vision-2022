...
def grid_points(img, nPointsX, nPointsY, border):
    ...
    # here a border is left on top, bottom, left and right
    ...
    for i in range(len(vPoints)):
        ...
        for cell_y in range(-2, 2):
            for cell_x in range(-2, 2):
                ...
                cell_xcoors, cell_ycoors = np.meshgrid(np.arange(start_x, end_x), np.arange(start_y, end_y))
                cell_xcoors = cell_xcoors.ravel() # the x coordinates of all pixels in the current cell --> [w*h,]
                cell_ycoors = cell_ycoors.ravel() # the y coordinates of all pixels in the current cell --> [w*h,]

                cell_grad_x = grad_x[cell_ycoors, cell_xcoors]
                cell_grad_y = grad_y[cell_ycoors, cell_xcoors] 
                cell_orients = np.arctan2(cell_grad_y,  cell_grad_x) # orientation = arctan(y / x) for each pixel
                cell_hist, _ = np.histogram(cell_orients, bins = nBins, range = (-np.pi, np.pi)) # compute the HOG of the current cell
...
def bow_histogram(vFeatures, vCenters):
    ...
    # todo
    k = vCenters.shape[0]
    cluster_id, _ = findnn(vFeatures, vCenters) # get the cluster assignment for each of M feature vectors
    histo = np.bincount(cluster_id, minlength = k) # set minlength to ensure the histgram is counting for all k cluster bins [k,]
...
def bow_recognition_nearest(histogram,vBoWPos,vBoWNeg):
    ...
    # todo
    _, DistPos = findnn(histogram, vBoWPos) # get the distance from the test img to its nearest positive traning sets
    _, DistNeg = findnn(histogram, vBoWNeg) # get the distance from the test img to its nearest negative traning sets
